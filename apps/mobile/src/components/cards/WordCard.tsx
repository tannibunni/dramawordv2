...
const WordCard: React.FC<WordCardProps> = ({
  ...
  // 日语转罗马音函数
  const convertToRomaji = (japaneseText: string): string => {
    // 简单的假名到罗马音映射
    ...
    // 片假名到罗马音映射（与平假名相同）
    const katakanaToRomaji: Record<string, string> = {
      'ア': 'a', 'イ': 'i', 'ウ': 'u', 'エ': 'e', 'オ': 'o',
    ...
    // 处理汉字和其他字符（保持原样）
    result = result.replace(/[一-龯]/g, (match) => {
      // 汉字保持原样，但用空格分隔
      return ` ${match} `;
    });
