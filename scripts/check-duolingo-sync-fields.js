...
async function checkDatabaseFields() {
  try {
    ...
    const syncFields = [
      ...
      { model: UserLearningRecord, name: 'UserLearningRecord', fields: ['userId', 'wordId', 'mastery', 'nextReviewDate'] },
      ...
      { model: UserShowList, name: 'UserShowList', fields: ['userId', 'showId', 'isWatching', 'progress'] },
      ...
      try {
        ...
        if (count > 0) {
          ...
          fields.forEach(field => {
            const value = field.split('.').reduce((obj, key) => obj?.[key], sample);
